// ============================================================================
// quantize_lut.cc - 分段线性查找表生成（纯 CPU 版本）
// ============================================================================
...
// 最小二乘线性拟合
void linear_fit(const std::vector<float>& x, const std::vector<float>& y, float& b, float& c) {
    ...
    if (std::abs(denom) < 1e-9f) denom = 1e-9f;
...
int8_t determine_shift_bits_int8(float max_val) {
...
int8_t determine_shift_bits_int16(float max_val) {
    if (max_val < 1e-9f) return 0;
...
int32_t quantize_coefficient_int32(float val_fp, int8_t shift_bits) {
...
int8_t quantize_input_int8(float val_fp, int8_t shift_bits, int32_t zp) {
    float scale = std::pow(2.0f, -static_cast<float>(shift_bits));
...
int16_t quantize_input_int16(float val_fp, int8_t shift_bits, int32_t zp) {
    float scale = std::pow(2.0f, -static_cast<float>(shift_bits));
    int32_t q = static_cast<int32_t>(std::round(val_fp / scale + static_cast<float>(zp)));
